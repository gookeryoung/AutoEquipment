...
**关键约束**：`Brawler` 角色永远不会被分配远程武器。在武器评分管线中，`WeaponTraitScorer` 检测到格斗者持远程武器时直接 `Veto(-9000f)`，管线短路。
...
| 2 | `WeaponTraitScorer` | 特质（含格斗者否决远程武器） |
| 3 | `WeaponSkillScorer` | 技能等级 × 兴趣乘数（无火 1.0 / 单火 1.5 / 双火 2.0） |
