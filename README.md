...
- **DEBUG 显示**：命中自定义评级的 Pawn 在面板与日志中显示完整识别码 `王五[S#王五]`；自动档仅显示 `王五[S]`
- **面板对比**：命中自定义档时面板同时显示"当前档（自定义）"与"自动档"，便于玩家对照
- **排序规则**：自定义评级档次映射为代表分（D=5, C=15, B=25, A=50, S=80, X=-1）+0.5 微量偏向，让同档自定义略优先于同档自动
- **覆盖范围**：`SidearmAllocator.GetCombatTier` 与 `ComputeCombatValue` 均先查自定义表，命中即跳过公式

**使用场景**：玩家认为某 S 档殖民者特别关键需强制锁为 S，或某 X 档小人有特殊价值需提升到 A。
