...
- **入口**：殖民者检视面板 `ITab_GearManager` 最下方两个并排按钮
  - **全局重配**：执行分配
  - **重配规则**：打开 `ReallocateRulesWindow`，查看规则说明与可调开关
- **流程**：
...
|------|------|
| 战斗价值降序 | 高价值殖民者优先评估 |
| 战斗价值公式 | 射击等级×兴趣乘数 + 近战等级×兴趣乘数 |
| Brawler 特质 | 坚决拒绝远程武器（Veto） |
| 双修角色 | 远程+近战均有火，主武器优先远程 |
| 纯近战角色 | 射击无火，副武器优先 EMP |
| 护盾腰带约束 | 带护盾腰带者不分配远程副武器 |
